...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const identifierValidation = validators.required(formData.identifier, "Nomor HP atau Email");
    if (!identifierValidation.isValid) {
      newErrors.identifier = identifierValidation.message!;
    } else if (!validators.emailOrPhone(formData.identifier)) {
      newErrors.identifier = "Format nomor HP (08xxx) atau email tidak valid";
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-500 font-poppins">
            Gunakan nomor HP (08xxx) atau email yang terdaftar
          </p>
        </div>
